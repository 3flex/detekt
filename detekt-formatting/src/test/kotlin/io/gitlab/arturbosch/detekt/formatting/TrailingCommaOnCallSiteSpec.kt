...
class TrailingCommaOnCallSiteSpec {
    ...
    inner class `unnecessary comma` {
        ...
        fun `reports unnecessary comma on function call`() {
            val code = """
                val foo1 = listOf("a", "b",)
            """.trimIndent()
            val findings = TrailingCommaOnCallSite(TestConfig(mapOf(ALLOW_TRAILING_COMMA_ON_CALL_SITE to false))).lint(code)
    ...
    inner class `missing comma` {
        ...
        fun `reports missing comma on function call`() {
            ...
            """.trimIndent()
            val findings = TrailingCommaOnCallSite(TestConfig(mapOf(ALLOW_TRAILING_COMMA_ON_CALL_SITE to true))).lint(code)
            assertThat(findings).hasSize(1)
        }
