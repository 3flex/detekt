...
class KotlinAnalysisAAEngineTest {
    ...
    fun `can compile a valid script`() {
        val code = """
            class A
        """.trimIndent()
        KotlinAnalysisAAEngine.compile(code)
    }
    ...
    fun `fails compiling an invalid script`() {
        ...
        """.trimIndent()
        assertThatThrownBy { KotlinAnalysisAAEngine.compile(invalidCode) }
    ...
    fun `fails on missing import if import used in previous script - #5739`() {
        ...
        KotlinAnalysisAAEngine.compile(validCode)
        ...
        assertThatThrownBy { KotlinAnalysisAAEngine.compile(codeWithMissingImport) }
            .isInstanceOf(IllegalStateException::class.java)
