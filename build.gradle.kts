...
tasks.create<JacocoReport>("rootJacocoTestReport") {
    val jacocoReportTasks =
        subprojects
            ...
            .map { it.tasks["jacocoTestReport"] as JacocoReport } // For Junit 4 use `.flatMap { it.tasks.withType<JacocoReport>() }`
    dependsOn(jacocoReportTasks) // For Junit 4 this becomes `dependsOn(tasks["test"])`

    val executionData = jacocoReportTasks.map { it.executionData }.toTypedArray()
